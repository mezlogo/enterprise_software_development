...
void test_alter() {
	...
	assertLongEquals("Should alterAVLTree",
					 ALTER_SUCCESS, alterAVLTree(&key1, &key5));
	...
	assertLongEquals("Shouldn remove", REMOVE_SUCCESS,
					 alterAVLTree(&key5, NULL));
	assertLongEquals("Shouldn't find", FIND_FAIL,
					 findAVLTree(&key5));

	assertLongEquals("Shouldn't alterAVLTree",
					 ALTER_FAIL, alterAVLTree(NULL, &key2));
	assertLongEquals("Shouldn't alterAVLTree",
					 ALTER_SUCCESS, alterAVLTree(&key5, &key3));
}
void test_find() {
	int nodesCount = 10;
	initMM(nodesCount);
...
void test_insert() {
	...
	while (nodesCount--) {
		assertLongEquals("Should inser", INSERT_SUCCESS,
						 insertAVLTree(&key));
	}
