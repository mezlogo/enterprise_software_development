...
void test_case() {
	...
	assertLongEquals("Tree is broken",
					 (unsigned long) &node3,
					 (unsigned long) node7.left);
	assertLongEquals("Tree is broken",
					 (unsigned long) &node9,
					 (unsigned long) node7.right);

	rotateRight(&node7);

	assertLongEquals("Tree is broken",
					 (unsigned long) &node1,
					 (unsigned long) node3.left);
	assertLongEquals("Tree is broken",
					 (unsigned long) &node7,
					 (unsigned long) node3.right);
	assertLongEquals("Tree is broken",
					 (unsigned long) &node5,
					 (unsigned long) node7.left);
	...
	assertLongEquals("Root changed",
					 (unsigned long) currentRoot,
					 (unsigned long) &node5);
	...
	currentRoot = insertAVLByNode(currentRoot,
								  &node10);

	assertLongEquals("Fix height", 4, node5.height);
	assertLongEquals("Fix height", 2,
					 node5.left->height);
	assertLongEquals("Fix height", 3,
					 node5.right->height);
	assertLongEquals("Fix height", 2, node9.height);
	assertLongEquals("Tree is broken",
					 (unsigned long) node9.right,
					 (unsigned long) &node10);
	assertLongEquals("Tree is broken",
					 (unsigned long) node9.left, (unsigned long) NULL);
	assertLongEquals("Tree is broken",
					 (unsigned long) currentRoot,
					 (unsigned long) &node5);
	...
	AVLNode* rmResult = removeAVLNode(&node5, &key10,
									  &deleteOp);
	...
	assertLongEquals("Tree is broken",
					 (unsigned long) findAVLNode(currentRoot,
							 node9.key), (unsigned long) &node9);

	Key missKey = {100, 0};
	assertLongEquals("Tree is broken",
					 (unsigned long) findAVLNode(currentRoot,
							 &missKey), (unsigned long) NULL);
	assertLongEquals("Tree is broken",
					 (unsigned long) findAVLNode(currentRoot, &key10),
					 (unsigned long) NULL);
}
