...
void test_message_handler() {
    ...
    msg[0] = 1;
    assertLongEquals("Should be valid", MESSAGE_VALID, handleMessage(msg) & 0xff);

    msg[0] = 2;
    assertLongEquals("Should be valid", MESSAGE_VALID, handleMessage(msg) & 0xff);

    msg[0] = 0;
    assertLongEquals("Should be invalid", MESSAGE_INVALID, handleMessage(msg) & 0xff);
    ...
    assertLongEquals("Should be called alter", 1, isWasAlter);
...
int main(int argc, char** argv) {
    testSuit("Message handler", 2,
	     initTestCase("Support features case", &test_support_features),
	     initTestCase("Message handler case", &test_message_handler)
	    );
    return 0;
}
